...
#if defined(CONFIG_SOC_SERIES_STM32H7X)
	const uint32_t cur_tx_desc_idx = heth->TxDescList.CurTxDesc;
...
#if defined(CONFIG_SOC_SERIES_STM32H7X)
		ETH_TxDescListTypeDef *dma_tx_desc_list = &heth->TxDescList;
		__IO ETH_DMADescTypeDef *last_dma_tx_desc;

		for (uint32_t i = 0; i < ETH_TX_DESC_CNT; i++) {
			uint32_t last_desc_idx = (cur_tx_desc_idx + i) % ETH_TX_DESC_CNT;
			last_dma_tx_desc =
...
static struct net_pkt *eth_rx(const struct device *dev, uint16_t *vlan_tag)
{
...
#if defined(CONFIG_PTP_CLOCK_STM32_HAL)
#if defined(CONFIG_SOC_SERIES_STM32H7X)
	ETH_RxDescListTypeDef *dma_rx_desc_list = &heth->RxDescList;
	if (dma_rx_desc_list->AppDescNbr) {
		...
		if (dma_rx_desc_list->AppContextDesc &&
			...
			uint32_t context_desc_idx = (last_desc_idx + 1U) % ETH_RX_DESC_CNT;
...
#else
	__IO ETH_DMADescTypeDef *last_dma_rx_desc = heth->RxFrameInfos.LSRxDesc;
...
static int ptp_stm32_init(const struct device *port)
{
	...
	eth_dev_data->clk_ratio =
		(double)CONFIG_ETH_STM32_HAL_PTP_CLOCK_SRC_HZ / ptp_hclk_rate;
	eth_dev_data->clk_ratio_adj = 1.0;
	uint32_t addend_val =
